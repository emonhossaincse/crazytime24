import React from "react";

export default function Inplay() {
  return (
    <div className="pl-PodLoaderModule_Pod-61  in-play">
      <div className="inplay-menu">
        <div className="prev">
          <svg
            baseProfile="tiny"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 42 42"
            xmlSpace="preserve"
            transform="scale(-1 1)"
            className="inplay-prev"
            fill="currentColor"
          >
            <path
              fill="currentColor"
              d="M13.933 1 34 21.068 14.431 40.637l-4.933-4.933 14.638-14.636L9 5.933z"
            />
          </svg>
        </div>
        <div className="next">
          <svg
            baseProfile="tiny"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 42 42"
            xmlSpace="preserve"
            className="inplay-next"
            fill="currentColor"
          >
            <path
              fill="currentColor"
              d="M13.933 1 34 21.068 14.431 40.637l-4.933-4.933 14.638-14.636L9 5.933z"
            />
          </svg>
        </div>
        <div className="inplay-menu-item">
          <svg
            viewBox="0 0 512 512"
            fill="currentColor"
            height="2.3rem"
            width="2.3rem"
          >
            <path d="M256 48C141.31 48 48 141.31 48 256s93.31 208 208 208 208-93.31 208-208S370.69 48 256 48zm143 304h-45.22a8 8 0 01-6.91-4l-16.14-27.68a8 8 0 01-.86-6l14.86-59.92a8 8 0 014.65-5.45l28.1-11.9a8 8 0 018.34 1.3l41.63 35.82a8 8 0 012.69 7.26 174.75 174.75 0 01-24.28 66.68A8 8 0 01399 352zM134.52 237.13l28.1 11.9a8 8 0 014.65 5.45l14.86 59.92a8 8 0 01-.86 6L165.13 348a8 8 0 01-6.91 4H113a8 8 0 01-6.82-3.81 174.75 174.75 0 01-24.28-66.68 8 8 0 012.69-7.26l41.63-35.82a8 8 0 018.3-1.3zm256.94-87.24l-18.07 51.38A8 8 0 01369 206l-29.58 12.53a8 8 0 01-8.26-1.24L274.9 170.1a8 8 0 01-2.9-6.1v-33.58a8 8 0 013.56-6.65l42.83-28.54a8 8 0 017.66-.67A176.92 176.92 0 01390 142a8 8 0 011.46 7.89zM193.6 95.23l42.84 28.54a8 8 0 013.56 6.65V164a8 8 0 01-2.86 6.13l-56.26 47.19a8 8 0 01-8.26 1.24L143 206a8 8 0 01-4.43-4.72L120.5 149.9a8 8 0 011.5-7.9 176.92 176.92 0 0164-47.48 8 8 0 017.6.71zm17.31 327.46L191.18 373a8 8 0 01.52-7l15.17-26a8 8 0 016.91-4h84.44a8 8 0 016.91 4l15.18 26a8 8 0 01.53 7l-19.59 49.67a8 8 0 01-5.69 4.87 176.58 176.58 0 01-79 0 8 8 0 01-5.65-4.85z" />
          </svg>
          <p className="inplay-menu-title">Football</p>
        </div>
        <div className="inplay-menu-item">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 465 465"
            xmlSpace="preserve"
          >
            <g transform="translate(0 -540.36)">
              <path d="M408.1 790.36c-4-.8-8 1.8-8.8 5.9-5.3 26.2-15.3 50.8-32.7 79.7-2.1 3.6-1 8.2 2.6 10.3 1.3.8 2.6 1.1 3.9 1.1 2.6 0 5-1.3 6.4-3.7 18.3-30.4 28.9-56.5 34.5-84.5.8-4-1.8-8-5.9-8.8m-120.425-20.88-10.606-10.608 24.89-24.89 10.607 10.607zM96.046 877.821 85.44 867.215l24.89-24.89 10.607 10.607zm51.444-73.028 10.607 10.606-24.89 24.89-10.607-10.607zm74.85-74.493 10.606 10.607-24.89 24.89-10.607-10.607zm-37.352 37.278 10.606 10.607-24.89 24.89-10.607-10.607zm74.502-74.768 10.607 10.606-24.89 24.89-10.607-10.607zm37.49-37.253 10.606 10.607-24.89 24.89-10.607-10.607zm37.221-37.42 10.606 10.607-24.89 24.89-10.607-10.607zM138.317 918.89l-10.606-10.606 24.89-24.89L163.209 894zm37.364-37.406-10.606-10.606 24.89-24.89 10.607 10.607zm74.644-74.599-10.606-10.607 24.89-24.89 10.607 10.607zm-37.294 37.337-10.606-10.607 24.89-24.89 10.607 10.607zM325.04 732.215l-10.607-10.607 24.891-24.89 10.606 10.607zm37.279-37.335-10.607-10.607 24.891-24.89 10.606 10.607z" />
              <path d="M396.7 608.16c-43.9-43.7-102.2-67.8-164.2-67.8-62.5 0-120.9 24.1-164.7 67.8C24.1 651.86 0 710.36 0 772.86c0 62 24.1 120.3 67.8 164.2 43.9 44 102.4 68.3 164.7 68.3 61.9 0 120.2-24.3 164.2-68.3s68.3-102.4 68.3-164.2c0-62.3-24.3-120.8-68.3-164.7M15 772.86c0-119.9 97.6-217.5 217.5-217.5 35.5 0 69 8.5 98.6 23.7L38.7 871.46c-15.1-29.6-23.7-63.1-23.7-98.6m91.8 177.4 19-19-10.6-10.6-20.4 20.4c-11.4-9.3-21.8-19.8-31.1-31.1l19.7-19.7-10.6-10.6-18.3 18.3c-2.9-4.1-5.7-8.4-8.3-12.8l298.5-298.5c4.4 2.6 8.6 5.4 12.8 8.3l-10.8 10.8 10.6 10.6 12.3-12.3c11.3 9.2 21.6 19.4 30.8 30.6l-11.6 12.3 10.9 10.3 9.9-10.5c2.9 4 5.6 8.1 8.2 12.3l-299.1 299.1c-4.1-2.5-8.1-5.1-11.9-7.9m125.7 40.1c-36.1 0-70.2-8.9-100.2-24.5l293.2-293.2c15.7 30 24.5 64.1 24.5 100.2 0 119.9-97.6 217.5-217.5 217.5" />
              <circle cx={357} cy={904.358} r={7.5} />
            </g>
          </svg>
          <p className="inplay-menu-title">Cricket</p>
        </div>
        <div className="inplay-menu-item">
          <svg data-name="Layer 1" viewBox="0 0 24 24" fill="currentColor">
            <path d="M12 2a10 10 0 00-8.4 4.59 10 10 0 0014.66 13.2A10 10 0 0012 2zm2 2.26A8 8 0 0119.74 10 9.78 9.78 0 0015 11.38a15.7 15.7 0 00-2.4-2.21A10.06 10.06 0 0014 4.26zM12 4a7.9 7.9 0 01-1.14 4.07c-.15-.08-.29-.17-.44-.24a15.52 15.52 0 00-4.09-1.47A8 8 0 0112 4zM5 8.16A13.75 13.75 0 019.49 9.6l.13.08A7.93 7.93 0 014 12a8 8 0 011-3.84zm5 11.58A8 8 0 014.26 14a9.9 9.9 0 007.08-3.21 14 14 0 012 1.8A10 10 0 0010 19.74zm2 .26a8 8 0 012.56-5.85c.06.08.12.15.17.23a14 14 0 011.84 4.18A7.93 7.93 0 0112 20zm6.21-3a16 16 0 00-1.8-3.75L16.2 13a7.93 7.93 0 013.8-1 8 8 0 01-1.79 5z" />
          </svg>
          <p className="inplay-menu-title">Basketball</p>
        </div>
        <div className="inplay-menu-item">
          <svg data-name="Layer 1" viewBox="0 0 24 24" fill="currentColor">
            <path d="M12 2a10 10 0 00-7.35 16.76l.09.1a10 10 0 0014.52 0l.09-.1A10 10 0 0012 2zM5.61 16.79a7.93 7.93 0 010-9.58 6 6 0 010 9.58zM12 20a7.91 7.91 0 01-5-1.77A8 8 0 007 5.77a7.95 7.95 0 0110 0 8 8 0 000 12.46A7.91 7.91 0 0112 20zm6.39-3.21a6 6 0 010-9.58 7.93 7.93 0 010 9.58z" />
          </svg>
          <p className="inplay-menu-title">Tennis</p>
        </div>
        <div className="inplay-menu-item">
          <svg
            viewBox="0 0 512 512"
            fill="currentColor"
            height="2.3rem"
            width="2.3rem"
          >
            <path d="M256 48C141.31 48 48 141.31 48 256s93.31 208 208 208 208-93.31 208-208S370.69 48 256 48zm143 304h-45.22a8 8 0 01-6.91-4l-16.14-27.68a8 8 0 01-.86-6l14.86-59.92a8 8 0 014.65-5.45l28.1-11.9a8 8 0 018.34 1.3l41.63 35.82a8 8 0 012.69 7.26 174.75 174.75 0 01-24.28 66.68A8 8 0 01399 352zM134.52 237.13l28.1 11.9a8 8 0 014.65 5.45l14.86 59.92a8 8 0 01-.86 6L165.13 348a8 8 0 01-6.91 4H113a8 8 0 01-6.82-3.81 174.75 174.75 0 01-24.28-66.68 8 8 0 012.69-7.26l41.63-35.82a8 8 0 018.3-1.3zm256.94-87.24l-18.07 51.38A8 8 0 01369 206l-29.58 12.53a8 8 0 01-8.26-1.24L274.9 170.1a8 8 0 01-2.9-6.1v-33.58a8 8 0 013.56-6.65l42.83-28.54a8 8 0 017.66-.67A176.92 176.92 0 01390 142a8 8 0 011.46 7.89zM193.6 95.23l42.84 28.54a8 8 0 013.56 6.65V164a8 8 0 01-2.86 6.13l-56.26 47.19a8 8 0 01-8.26 1.24L143 206a8 8 0 01-4.43-4.72L120.5 149.9a8 8 0 011.5-7.9 176.92 176.92 0 0164-47.48 8 8 0 017.6.71zm17.31 327.46L191.18 373a8 8 0 01.52-7l15.17-26a8 8 0 016.91-4h84.44a8 8 0 016.91 4l15.18 26a8 8 0 01.53 7l-19.59 49.67a8 8 0 01-5.69 4.87 176.58 176.58 0 01-79 0 8 8 0 01-5.65-4.85z" />
          </svg>
          <p className="inplay-menu-title">Football</p>
        </div>
        <div className="inplay-menu-item">
          <svg
            viewBox="0 0 512 512"
            fill="currentColor"
            height="2.3rem"
            width="2.3rem"
          >
            <path d="M256 48C141.31 48 48 141.31 48 256s93.31 208 208 208 208-93.31 208-208S370.69 48 256 48zm143 304h-45.22a8 8 0 01-6.91-4l-16.14-27.68a8 8 0 01-.86-6l14.86-59.92a8 8 0 014.65-5.45l28.1-11.9a8 8 0 018.34 1.3l41.63 35.82a8 8 0 012.69 7.26 174.75 174.75 0 01-24.28 66.68A8 8 0 01399 352zM134.52 237.13l28.1 11.9a8 8 0 014.65 5.45l14.86 59.92a8 8 0 01-.86 6L165.13 348a8 8 0 01-6.91 4H113a8 8 0 01-6.82-3.81 174.75 174.75 0 01-24.28-66.68 8 8 0 012.69-7.26l41.63-35.82a8 8 0 018.3-1.3zm256.94-87.24l-18.07 51.38A8 8 0 01369 206l-29.58 12.53a8 8 0 01-8.26-1.24L274.9 170.1a8 8 0 01-2.9-6.1v-33.58a8 8 0 013.56-6.65l42.83-28.54a8 8 0 017.66-.67A176.92 176.92 0 01390 142a8 8 0 011.46 7.89zM193.6 95.23l42.84 28.54a8 8 0 013.56 6.65V164a8 8 0 01-2.86 6.13l-56.26 47.19a8 8 0 01-8.26 1.24L143 206a8 8 0 01-4.43-4.72L120.5 149.9a8 8 0 011.5-7.9 176.92 176.92 0 0164-47.48 8 8 0 017.6.71zm17.31 327.46L191.18 373a8 8 0 01.52-7l15.17-26a8 8 0 016.91-4h84.44a8 8 0 016.91 4l15.18 26a8 8 0 01.53 7l-19.59 49.67a8 8 0 01-5.69 4.87 176.58 176.58 0 01-79 0 8 8 0 01-5.65-4.85z" />
          </svg>
          <p className="inplay-menu-title">Football</p>
        </div>
        <div className="inplay-menu-item">
          <svg
            viewBox="0 0 512 512"
            fill="currentColor"
            height="2.3rem"
            width="2.3rem"
          >
            <path d="M256 48C141.31 48 48 141.31 48 256s93.31 208 208 208 208-93.31 208-208S370.69 48 256 48zm143 304h-45.22a8 8 0 01-6.91-4l-16.14-27.68a8 8 0 01-.86-6l14.86-59.92a8 8 0 014.65-5.45l28.1-11.9a8 8 0 018.34 1.3l41.63 35.82a8 8 0 012.69 7.26 174.75 174.75 0 01-24.28 66.68A8 8 0 01399 352zM134.52 237.13l28.1 11.9a8 8 0 014.65 5.45l14.86 59.92a8 8 0 01-.86 6L165.13 348a8 8 0 01-6.91 4H113a8 8 0 01-6.82-3.81 174.75 174.75 0 01-24.28-66.68 8 8 0 012.69-7.26l41.63-35.82a8 8 0 018.3-1.3zm256.94-87.24l-18.07 51.38A8 8 0 01369 206l-29.58 12.53a8 8 0 01-8.26-1.24L274.9 170.1a8 8 0 01-2.9-6.1v-33.58a8 8 0 013.56-6.65l42.83-28.54a8 8 0 017.66-.67A176.92 176.92 0 01390 142a8 8 0 011.46 7.89zM193.6 95.23l42.84 28.54a8 8 0 013.56 6.65V164a8 8 0 01-2.86 6.13l-56.26 47.19a8 8 0 01-8.26 1.24L143 206a8 8 0 01-4.43-4.72L120.5 149.9a8 8 0 011.5-7.9 176.92 176.92 0 0164-47.48 8 8 0 017.6.71zm17.31 327.46L191.18 373a8 8 0 01.52-7l15.17-26a8 8 0 016.91-4h84.44a8 8 0 016.91 4l15.18 26a8 8 0 01.53 7l-19.59 49.67a8 8 0 01-5.69 4.87 176.58 176.58 0 01-79 0 8 8 0 01-5.65-4.85z" />
          </svg>
          <p className="inplay-menu-title">Football</p>
        </div>
      </div>
      <div className="ss-HomepageSpotlight ss-HomepageSpotlight_Width-1">
        <div className="ss-HomeSpotlightHeader">
          <div
            style={{ backgroundImage: "url(./icon/zClass_Cricket.svg)" }}
            className="ss-HomeSpotlightHeader_HeaderImage"
          />
          <div className="ss-HomeSpotlightHeader_Title ss-HomeSpotlightHeader_Title-lang1">
            Test Cricket Highlights
          </div>
        </div>
        <div className="ss-HomepageSpotlight_Modules">
          <div className="ss-HomepageSpotlight_Pod ss-HomepageSpotlight_CouponPodModuleContainer">
            <div className="cpm-CouponPodModule cpm-CouponPodModule-1 cpm-CouponPodModule-spotlight">
              <div className="cpm-Header">
                <div
                  style={{
                    backgroundImage: "url(./icon/zClass_Cricket.svg)"
                  }}
                  className="cpm-Header_HeaderImage"
                />
                <h2 className="cpm-Header_Title cpm-Header_Title-lang1">
                  Test Cricket Highlights
                </h2>
                <div className="cpm-Header_LinkContainer">
                  <div className="cpm-Header_Link" />
                </div>
              </div>
              <div className="cpm-OffersBar">
                {/**/}
              </div>
              <div className="cpm-CouponPodMarketGrid gl-MarketGrid">
                <div className="gl-MarketGroup cpm-CouponPodMarketGroup">
                  <div className="gl-MarketGroup_Wrapper">
                    <div className="gl-MarketGroupContainer cpm-CouponPodMarketGroup_MarketGroupContainer">
                      <div className="cpm-MarketFixture rcl-MarketFixtureDetailsLabelBase gl-Market_General gl-Market_General-columnheader gl-Market_General-haslabels">
                        <div className="cpm-MarketFixtureDateHeader cpm-MarketFixtureDateHeader-isdate">
                          Wed 27 Nov
                        </div>
                        <div className="cpm-ParticipantFixtureDetails rcl-MarketCouponAdvancedBase_LastChild gl-Market_General-cn1">
                          <div className="cpm-ParticipantFixtureDetails_LhsContainer">
                            <div className="cpm-ParticipantFixtureDetails_TeamsContainer">
                              <div className="cpm-ParticipantFixtureDetails_TeamNames">
                                <div className="cpm-ParticipantFixtureDetails_TeamContainer">
                                  <div className="cpm-ParticipantFixtureDetails_TeamKitContainer">
                                    <div className="tk-TeamKit tk-TeamKit-10 tk-TeamKit-10-2">
                                      <svg
                                        className="tk-TeamKit_SVG"
                                        width={48}
                                        height={44}
                                        viewBox="0 0 48 44"
                                        xmlns="http://www.w3.org/2000/svg"
                                        xmlnsXlink="http://www.w3.org/1999/xlink"
                                      >
                                        <defs>
                                          <path
                                            d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                            id="K10-2-path-1"
                                          />
                                          <mask
                                            id="K10-2-mask-2"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                          <mask
                                            id="K10-2-mask-3"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                        </defs>
                                        <g
                                          id="SVG-Team-Kits-Separated"
                                          fill="none"
                                          fillRule="evenodd"
                                        >
                                          <g id="K10-2">
                                            <path
                                              d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                              id="color1"
                                              fill="#F0F0F0"
                                            />
                                            <path
                                              d="M16 3.05V44h-4l.004-40L16 3.05zm16 .055l4 .905V44h-4V3.105z"
                                              id="color2"
                                              fill="#F0F0F0"
                                            />
                                            <g id="hires-detail">
                                              <g id="Outline7">
                                                <g id="hires-detail">
                                                  <use
                                                    strokeOpacity=".07"
                                                    stroke="#FFF"
                                                    mask="url(#K10-2-mask-2)"
                                                    strokeWidth={4}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                  <use
                                                    stroke="#272727"
                                                    mask="url(#K10-2-mask-3)"
                                                    strokeWidth={2}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                </g>
                                              </g>
                                            </g>
                                          </g>
                                        </g>
                                      </svg>
                                    </div>
                                  </div>
                                  <div className="cpm-ParticipantFixtureDetails_TeamInfoWrapper">
                                    <div className="cpm-ParticipantFixtureDetails_Team">
                                      South Africa
                                    </div>
                                  </div>
                                </div>
                                <div className="cpm-ParticipantFixtureDetails_TeamContainer">
                                  <div className="cpm-ParticipantFixtureDetails_TeamKitContainer">
                                    <div className="tk-TeamKit tk-TeamKit-10 tk-TeamKit-10-2">
                                      <svg
                                        className="tk-TeamKit_SVG"
                                        width={48}
                                        height={44}
                                        viewBox="0 0 48 44"
                                        xmlns="http://www.w3.org/2000/svg"
                                        xmlnsXlink="http://www.w3.org/1999/xlink"
                                      >
                                        <defs>
                                          <path
                                            d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                            id="K10-2-path-1"
                                          />
                                          <mask
                                            id="K10-2-mask-2"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                          <mask
                                            id="K10-2-mask-3"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                        </defs>
                                        <g
                                          id="SVG-Team-Kits-Separated"
                                          fill="none"
                                          fillRule="evenodd"
                                        >
                                          <g id="K10-2">
                                            <path
                                              d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                              id="color1"
                                              fill="#F0F0F0"
                                            />
                                            <path
                                              d="M16 3.05V44h-4l.004-40L16 3.05zm16 .055l4 .905V44h-4V3.105z"
                                              id="color2"
                                              fill="#F0F0F0"
                                            />
                                            <g id="hires-detail">
                                              <g id="Outline7">
                                                <g id="hires-detail">
                                                  <use
                                                    strokeOpacity=".07"
                                                    stroke="#FFF"
                                                    mask="url(#K10-2-mask-2)"
                                                    strokeWidth={4}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                  <use
                                                    stroke="#272727"
                                                    mask="url(#K10-2-mask-3)"
                                                    strokeWidth={2}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                </g>
                                              </g>
                                            </g>
                                          </g>
                                        </g>
                                      </svg>
                                    </div>
                                  </div>
                                  <div className="cpm-ParticipantFixtureDetails_TeamInfoWrapper">
                                    <div className="cpm-ParticipantFixtureDetails_Team">
                                      Sri Lanka
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="cpm-ParticipantFixtureDetails_Details">
                              <div className="cpm-ParticipantFixtureDetails_BookCloses">
                                13:30
                              </div>
                              <div className="cpm-FixtureIconBlock">
                                <div className="cpm-FixtureIconBlock_VideoIcon" />
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          className="cpm-MarketFixtureDateHeader-isdate cpm-MarketFixtureDateHeader"
                          style={{}}
                        >
                          Thu 28 Nov
                        </div>
                        <div className="cpm-ParticipantFixtureDetails gl-Market_General-cn1">
                          <div className="cpm-ParticipantFixtureDetails_LhsContainer">
                            <div className="cpm-ParticipantFixtureDetails_TeamsContainer">
                              <div className="cpm-ParticipantFixtureDetails_TeamNames">
                                <div className="cpm-ParticipantFixtureDetails_TeamContainer">
                                  <div className="cpm-ParticipantFixtureDetails_TeamKitContainer">
                                    <div className="tk-TeamKit tk-TeamKit-10 tk-TeamKit-10-2">
                                      <svg
                                        className="tk-TeamKit_SVG"
                                        width={48}
                                        height={44}
                                        viewBox="0 0 48 44"
                                        xmlns="http://www.w3.org/2000/svg"
                                        xmlnsXlink="http://www.w3.org/1999/xlink"
                                      >
                                        <defs>
                                          <path
                                            d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                            id="K10-2-path-1"
                                          />
                                          <mask
                                            id="K10-2-mask-2"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                          <mask
                                            id="K10-2-mask-3"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                        </defs>
                                        <g
                                          id="SVG-Team-Kits-Separated"
                                          fill="none"
                                          fillRule="evenodd"
                                        >
                                          <g id="K10-2">
                                            <path
                                              d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                              id="color1"
                                              fill="#F0F0F0"
                                            />
                                            <path
                                              d="M16 3.05V44h-4l.004-40L16 3.05zm16 .055l4 .905V44h-4V3.105z"
                                              id="color2"
                                              fill="#F0F0F0"
                                            />
                                            <g id="hires-detail">
                                              <g id="Outline7">
                                                <g id="hires-detail">
                                                  <use
                                                    strokeOpacity=".07"
                                                    stroke="#FFF"
                                                    mask="url(#K10-2-mask-2)"
                                                    strokeWidth={4}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                  <use
                                                    stroke="#272727"
                                                    mask="url(#K10-2-mask-3)"
                                                    strokeWidth={2}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                </g>
                                              </g>
                                            </g>
                                          </g>
                                        </g>
                                      </svg>
                                    </div>
                                  </div>
                                  <div className="cpm-ParticipantFixtureDetails_TeamInfoWrapper">
                                    <div className="cpm-ParticipantFixtureDetails_Team">
                                      New Zealand
                                    </div>
                                  </div>
                                </div>
                                <div className="cpm-ParticipantFixtureDetails_TeamContainer">
                                  <div className="cpm-ParticipantFixtureDetails_TeamKitContainer">
                                    <div className="tk-TeamKit tk-TeamKit-10 tk-TeamKit-10-2">
                                      <svg
                                        className="tk-TeamKit_SVG"
                                        width={48}
                                        height={44}
                                        viewBox="0 0 48 44"
                                        xmlns="http://www.w3.org/2000/svg"
                                        xmlnsXlink="http://www.w3.org/1999/xlink"
                                      >
                                        <defs>
                                          <path
                                            d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                            id="K10-2-path-1"
                                          />
                                          <mask
                                            id="K10-2-mask-2"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                          <mask
                                            id="K10-2-mask-3"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                        </defs>
                                        <g
                                          id="SVG-Team-Kits-Separated"
                                          fill="none"
                                          fillRule="evenodd"
                                        >
                                          <g id="K10-2">
                                            <path
                                              d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                              id="color1"
                                              fill="#F0F0F0"
                                            />
                                            <path
                                              d="M16 3.05V44h-4l.004-40L16 3.05zm16 .055l4 .905V44h-4V3.105z"
                                              id="color2"
                                              fill="#F0F0F0"
                                            />
                                            <g id="hires-detail">
                                              <g id="Outline7">
                                                <g id="hires-detail">
                                                  <use
                                                    strokeOpacity=".07"
                                                    stroke="#FFF"
                                                    mask="url(#K10-2-mask-2)"
                                                    strokeWidth={4}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                  <use
                                                    stroke="#272727"
                                                    mask="url(#K10-2-mask-3)"
                                                    strokeWidth={2}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                </g>
                                              </g>
                                            </g>
                                          </g>
                                        </g>
                                      </svg>
                                    </div>
                                  </div>
                                  <div className="cpm-ParticipantFixtureDetails_TeamInfoWrapper">
                                    <div className="cpm-ParticipantFixtureDetails_Team">
                                      England
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="cpm-ParticipantFixtureDetails_Details">
                              <div className="cpm-ParticipantFixtureDetails_BookCloses">
                                04:00
                              </div>
                              <div className="cpm-FixtureIconBlock">
                                <div className="cpm-FixtureIconBlock_AudioIcon" />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="cpm-MarketOdds gl-Market_General gl-Market_General-columnheader">
                        <div className="cpm-MarketOddsHeader">1</div>
                        <div className="cpm-ParticipantOdds gl-Participant_General cpm-ParticipantOddsOnly100 rcl-MarketCouponAdvancedBase_LastChild gl-Market_General-cn1 cpm-ParticipantOdds_Suspended">
                          <span className="cpm-ParticipantOdds_Odds">1.61</span>
                        </div>
                        <div className="cpm-MarketOddsHeader">1</div>
                        <div className="cpm-ParticipantOdds gl-Participant_General cpm-ParticipantOddsOnly100 gl-Market_General-cn1">
                          <span className="cpm-ParticipantOdds_Odds">1.90</span>
                        </div>
                      </div>
                      <div className="cpm-MarketOdds gl-Market_General gl-Market_General-columnheader">
                        <div className="cpm-MarketOddsHeader">X</div>
                        <div className="cpm-ParticipantOdds gl-Participant_General cpm-ParticipantOddsOnly100 rcl-MarketCouponAdvancedBase_LastChild gl-Market_General-cn1 cpm-ParticipantOdds_Suspended">
                          <span className="cpm-ParticipantOdds_Odds">5.00</span>
                        </div>
                        <div className="cpm-MarketOddsHeader">X</div>
                        <div className="cpm-ParticipantOdds gl-Participant_General cpm-ParticipantOddsOnly100 gl-Market_General-cn1">
                          <span className="cpm-ParticipantOdds_Odds">
                            13.00
                          </span>
                        </div>
                      </div>
                      <div className="cpm-MarketOdds gl-Market_General gl-Market_General-columnheader">
                        <div className="cpm-MarketOddsHeader">2</div>
                        <div className="cpm-ParticipantOdds gl-Participant_General cpm-ParticipantOddsOnly100 rcl-MarketCouponAdvancedBase_LastChild gl-Market_General-cn1 cpm-ParticipantOdds_Suspended">
                          <span className="cpm-ParticipantOdds_Odds">4.33</span>
                        </div>
                        <div className="cpm-MarketOddsHeader">2</div>
                        <div className="cpm-ParticipantOdds gl-Participant_General cpm-ParticipantOddsOnly100 gl-Market_General-cn1">
                          <span className="cpm-ParticipantOdds_Odds">2.20</span>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      {/* </div>
      <div className="ss-HomepageSpotlight ss-HomepageSpotlight_Width-1"> */}
        <div className="ss-HomeSpotlightHeader">
          <div
            style={{ backgroundImage: "url(./icon/zClass_Cricket.svg)" }}
            className="ss-HomeSpotlightHeader_HeaderImage"
          />
          <div className="ss-HomeSpotlightHeader_Title ss-HomeSpotlightHeader_Title-lang1">
            Cricket Highlights
          </div>
        </div>
        <div className="ss-HomepageSpotlight_Modules">
          <div className="ss-HomepageSpotlight_Pod ss-HomepageSpotlight_CouponPodModuleContainer">
            <div className="cpm-CouponPodModule cpm-CouponPodModule-1 cpm-CouponPodModule-spotlight">
              <div className="cpm-Header">
                <div style={{ backgroundImage: "url(./icon/zClass_Cricket.svg)"
                  }}
                  className="cpm-Header_HeaderImage"
                />
                <h2 className="cpm-Header_Title cpm-Header_Title-lang1">
                  Cricket Highlights
                </h2>
                <div className="cpm-Header_LinkContainer">
                  <div className="cpm-Header_Link" />
                </div>
              </div>
              <div className="cpm-OffersBar">
                {/**/}
              </div>
              <div className="cpm-CouponPodMarketGrid gl-MarketGrid">
                <div className="gl-MarketGroup cpm-CouponPodMarketGroup">
                  <div className="gl-MarketGroup_Wrapper">
                    <div className="gl-MarketGroupContainer cpm-CouponPodMarketGroup_MarketGroupContainer">
                      <div className="cpm-MarketFixture rcl-MarketFixtureDetailsLabelBase gl-Market_General gl-Market_General-columnheader gl-Market_General-haslabels">
                        <div className="cpm-MarketFixtureDateHeader cpm-MarketFixtureDateHeader-isdate">
                          Wed 27 Nov
                        </div>
                        <div className="cpm-ParticipantFixtureDetails rcl-MarketCouponAdvancedBase_LastChild gl-Market_General-cn1">
                          <div className="cpm-ParticipantFixtureDetails_LhsContainer">
                            <div className="cpm-ParticipantFixtureDetails_TeamsContainer">
                              <div className="cpm-ParticipantFixtureDetails_TeamNames">
                                <div className="cpm-ParticipantFixtureDetails_TeamContainer">
                                  <div className="cpm-ParticipantFixtureDetails_TeamKitContainer">
                                    <div className="tk-TeamKit tk-TeamKit-10 tk-TeamKit-10-2">
                                      <svg
                                        className="tk-TeamKit_SVG"
                                        width={48}
                                        height={44}
                                        viewBox="0 0 48 44"
                                        xmlns="http://www.w3.org/2000/svg"
                                        xmlnsXlink="http://www.w3.org/1999/xlink"
                                      >
                                        <defs>
                                          <path
                                            d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                            id="K10-2-path-1"
                                          />
                                          <mask
                                            id="K10-2-mask-2"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                          <mask
                                            id="K10-2-mask-3"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                        </defs>
                                        <g
                                          id="SVG-Team-Kits-Separated"
                                          fill="none"
                                          fillRule="evenodd"
                                        >
                                          <g id="K10-2">
                                            <path
                                              d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                              id="color1"
                                              fill="#F0F0F0"
                                            />
                                            <path
                                              d="M16 3.05V44h-4l.004-40L16 3.05zm16 .055l4 .905V44h-4V3.105z"
                                              id="color2"
                                              fill="#F0F0F0"
                                            />
                                            <g id="hires-detail">
                                              <g id="Outline7">
                                                <g id="hires-detail">
                                                  <use
                                                    strokeOpacity=".07"
                                                    stroke="#FFF"
                                                    mask="url(#K10-2-mask-2)"
                                                    strokeWidth={4}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                  <use
                                                    stroke="#272727"
                                                    mask="url(#K10-2-mask-3)"
                                                    strokeWidth={2}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                </g>
                                              </g>
                                            </g>
                                          </g>
                                        </g>
                                      </svg>
                                    </div>
                                  </div>
                                  <div className="cpm-ParticipantFixtureDetails_TeamInfoWrapper">
                                    <div className="cpm-ParticipantFixtureDetails_Team">
                                      South Africa
                                    </div>
                                  </div>
                                </div>
                                <div className="cpm-ParticipantFixtureDetails_TeamContainer">
                                  <div className="cpm-ParticipantFixtureDetails_TeamKitContainer">
                                    <div className="tk-TeamKit tk-TeamKit-10 tk-TeamKit-10-2">
                                      <svg
                                        className="tk-TeamKit_SVG"
                                        width={48}
                                        height={44}
                                        viewBox="0 0 48 44"
                                        xmlns="http://www.w3.org/2000/svg"
                                        xmlnsXlink="http://www.w3.org/1999/xlink"
                                      >
                                        <defs>
                                          <path
                                            d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                            id="K10-2-path-1"
                                          />
                                          <mask
                                            id="K10-2-mask-2"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                          <mask
                                            id="K10-2-mask-3"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                        </defs>
                                        <g
                                          id="SVG-Team-Kits-Separated"
                                          fill="none"
                                          fillRule="evenodd"
                                        >
                                          <g id="K10-2">
                                            <path
                                              d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                              id="color1"
                                              fill="#F0F0F0"
                                            />
                                            <path
                                              d="M16 3.05V44h-4l.004-40L16 3.05zm16 .055l4 .905V44h-4V3.105z"
                                              id="color2"
                                              fill="#F0F0F0"
                                            />
                                            <g id="hires-detail">
                                              <g id="Outline7">
                                                <g id="hires-detail">
                                                  <use
                                                    strokeOpacity=".07"
                                                    stroke="#FFF"
                                                    mask="url(#K10-2-mask-2)"
                                                    strokeWidth={4}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                  <use
                                                    stroke="#272727"
                                                    mask="url(#K10-2-mask-3)"
                                                    strokeWidth={2}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                </g>
                                              </g>
                                            </g>
                                          </g>
                                        </g>
                                      </svg>
                                    </div>
                                  </div>
                                  <div className="cpm-ParticipantFixtureDetails_TeamInfoWrapper">
                                    <div className="cpm-ParticipantFixtureDetails_Team">
                                      Sri Lanka
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="cpm-ParticipantFixtureDetails_Details">
                              <div className="cpm-ParticipantFixtureDetails_BookCloses">
                                13:30
                              </div>
                              <div className="cpm-FixtureIconBlock">
                                <div className="cpm-FixtureIconBlock_VideoIcon" />
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          className="cpm-MarketFixtureDateHeader-isdate cpm-MarketFixtureDateHeader"
                          style={{}}
                        >
                          Thu 28 Nov
                        </div>
                        <div className="cpm-ParticipantFixtureDetails gl-Market_General-cn1">
                          <div className="cpm-ParticipantFixtureDetails_LhsContainer">
                            <div className="cpm-ParticipantFixtureDetails_TeamsContainer">
                              <div className="cpm-ParticipantFixtureDetails_TeamNames">
                                <div className="cpm-ParticipantFixtureDetails_TeamContainer">
                                  <div className="cpm-ParticipantFixtureDetails_TeamKitContainer">
                                    <div className="tk-TeamKit tk-TeamKit-10 tk-TeamKit-10-2">
                                      <svg
                                        className="tk-TeamKit_SVG"
                                        width={48}
                                        height={44}
                                        viewBox="0 0 48 44"
                                        xmlns="http://www.w3.org/2000/svg"
                                        xmlnsXlink="http://www.w3.org/1999/xlink"
                                      >
                                        <defs>
                                          <path
                                            d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                            id="K10-2-path-1"
                                          />
                                          <mask
                                            id="K10-2-mask-2"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                          <mask
                                            id="K10-2-mask-3"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                        </defs>
                                        <g
                                          id="SVG-Team-Kits-Separated"
                                          fill="none"
                                          fillRule="evenodd"
                                        >
                                          <g id="K10-2">
                                            <path
                                              d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                              id="color1"
                                              fill="#F0F0F0"
                                            />
                                            <path
                                              d="M16 3.05V44h-4l.004-40L16 3.05zm16 .055l4 .905V44h-4V3.105z"
                                              id="color2"
                                              fill="#F0F0F0"
                                            />
                                            <g id="hires-detail">
                                              <g id="Outline7">
                                                <g id="hires-detail">
                                                  <use
                                                    strokeOpacity=".07"
                                                    stroke="#FFF"
                                                    mask="url(#K10-2-mask-2)"
                                                    strokeWidth={4}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                  <use
                                                    stroke="#272727"
                                                    mask="url(#K10-2-mask-3)"
                                                    strokeWidth={2}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                </g>
                                              </g>
                                            </g>
                                          </g>
                                        </g>
                                      </svg>
                                    </div>
                                  </div>
                                  <div className="cpm-ParticipantFixtureDetails_TeamInfoWrapper">
                                    <div className="cpm-ParticipantFixtureDetails_Team">
                                      New Zealand
                                    </div>
                                  </div>
                                </div>
                                <div className="cpm-ParticipantFixtureDetails_TeamContainer">
                                  <div className="cpm-ParticipantFixtureDetails_TeamKitContainer">
                                    <div className="tk-TeamKit tk-TeamKit-10 tk-TeamKit-10-2">
                                      <svg
                                        className="tk-TeamKit_SVG"
                                        width={48}
                                        height={44}
                                        viewBox="0 0 48 44"
                                        xmlns="http://www.w3.org/2000/svg"
                                        xmlnsXlink="http://www.w3.org/1999/xlink"
                                      >
                                        <defs>
                                          <path
                                            d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                            id="K10-2-path-1"
                                          />
                                          <mask
                                            id="K10-2-mask-2"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                          <mask
                                            id="K10-2-mask-3"
                                            x={0}
                                            y={0}
                                            width={48}
                                            height={44}
                                            fill="#fff"
                                          >
                                            <use xlinkHref="#K10-2-path-1" />
                                          </mask>
                                        </defs>
                                        <g
                                          id="SVG-Team-Kits-Separated"
                                          fill="none"
                                          fillRule="evenodd"
                                        >
                                          <g id="K10-2">
                                            <path
                                              d="M36 4l12 11.636-6.384 6.717L36 17.48V44H12V17.48l-5.374 4.6L0 15.525 12 4l5.892-1.406 1.24-2.001C20.799.305 21.835 0 23.859 0c2.063 0 2.308.21 4.662.448l1.586 2.226L36 4z"
                                              id="color1"
                                              fill="#F0F0F0"
                                            />
                                            <path
                                              d="M16 3.05V44h-4l.004-40L16 3.05zm16 .055l4 .905V44h-4V3.105z"
                                              id="color2"
                                              fill="#F0F0F0"
                                            />
                                            <g id="hires-detail">
                                              <g id="Outline7">
                                                <g id="hires-detail">
                                                  <use
                                                    strokeOpacity=".07"
                                                    stroke="#FFF"
                                                    mask="url(#K10-2-mask-2)"
                                                    strokeWidth={4}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                  <use
                                                    stroke="#272727"
                                                    mask="url(#K10-2-mask-3)"
                                                    strokeWidth={2}
                                                    xlinkHref="#K10-2-path-1"
                                                  />
                                                </g>
                                              </g>
                                            </g>
                                          </g>
                                        </g>
                                      </svg>
                                    </div>
                                  </div>
                                  <div className="cpm-ParticipantFixtureDetails_TeamInfoWrapper">
                                    <div className="cpm-ParticipantFixtureDetails_Team">
                                      England
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="cpm-ParticipantFixtureDetails_Details">
                              <div className="cpm-ParticipantFixtureDetails_BookCloses">
                                04:00
                              </div>
                              <div className="cpm-FixtureIconBlock">
                                <div className="cpm-FixtureIconBlock_AudioIcon" />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="cpm-MarketOdds gl-Market_General gl-Market_General-columnheader">
                        <div className="cpm-MarketOddsHeader">1</div>
                        <div className="cpm-ParticipantOdds gl-Participant_General cpm-ParticipantOddsOnly100 rcl-MarketCouponAdvancedBase_LastChild gl-Market_General-cn1 cpm-ParticipantOdds_Suspended">
                          <span className="cpm-ParticipantOdds_Odds">1.61</span>
                        </div>
                        <div className="cpm-MarketOddsHeader">1</div>
                        <div className="cpm-ParticipantOdds gl-Participant_General cpm-ParticipantOddsOnly100 gl-Market_General-cn1">
                          <span className="cpm-ParticipantOdds_Odds">1.90</span>
                        </div>
                      </div>
                      <div className="cpm-MarketOdds gl-Market_General gl-Market_General-columnheader">
                        <div className="cpm-MarketOddsHeader">X</div>
                        <div className="cpm-ParticipantOdds gl-Participant_General cpm-ParticipantOddsOnly100 rcl-MarketCouponAdvancedBase_LastChild gl-Market_General-cn1 cpm-ParticipantOdds_Suspended">
                          <span className="cpm-ParticipantOdds_Odds">5.00</span>
                        </div>
                        <div className="cpm-MarketOddsHeader">X</div>
                        <div className="cpm-ParticipantOdds gl-Participant_General cpm-ParticipantOddsOnly100 gl-Market_General-cn1">
                          <span className="cpm-ParticipantOdds_Odds">
                            13.00
                          </span>
                        </div>
                      </div>
                      <div className="cpm-MarketOdds gl-Market_General gl-Market_General-columnheader">
                        <div className="cpm-MarketOddsHeader">2</div>
                        <div className="cpm-ParticipantOdds gl-Participant_General cpm-ParticipantOddsOnly100 rcl-MarketCouponAdvancedBase_LastChild gl-Market_General-cn1 cpm-ParticipantOdds_Suspended">
                          <span className="cpm-ParticipantOdds_Odds">4.33</span>
                        </div>
                        <div className="cpm-MarketOddsHeader">2</div>
                        <div className="cpm-ParticipantOdds gl-Participant_General cpm-ParticipantOddsOnly100 gl-Market_General-cn1">
                          <span className="cpm-ParticipantOdds_Odds">2.20</span>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
